...
var
  ...
  Control: TControl3D;
begin
  Value := TArcDial(Sender).Value;
  if cbCamera.IsChecked then
    Control := CameraUI
  else
    Control := Cube1;
  case TArcDial(Sender).Tag of
    0: Control.RotationAngle.X := Value;
    1: Control.RotationAngle.Y := Value;
    2: Control.RotationAngle.Z := Value;
  end;
end;
...
begin
  ...
  ArcDial1Change(ArcDial1);
  ArcDial1Change(ArcDial2);
  ArcDial1Change(ArcDial3);
end;
